...
(defun dfh-preceding-string (len)
  (if (bobp)
      ""
...
(defun dfh-following-string (len)
  (if (eobp)
      ""
...
;; macro ???
(defun dfh-following-string-at (pos len)
  (if (eobp)
      ""
...
(defun dfh-string-after (p len)
  (save-excursion
    (goto-char p)
    (dfh-following-string len)))

(defun dfh-string-before (p len)
  (save-excursion
    (goto-char p)
    ...
    (if strings
        (catch 'ret
          (dolist (mstr strings ret)
            (let* ((lstr (length mstr)) (fstr (dfh-following-string lstr)))
